...
class KVCacheModel():
    ...
    def _forward_with_kvcache(self, input_ids : torch.Tensor, decoder_input_ids: torch.Tensor, use_debug = True) -> torch.Tensor:
        if self._past_key_values is None:
            assert self._prob_history is None, f"{self._prob_history.shape}"
            # the first forward (prefill) returns the prompt's logits
            outputs = self._model(input_ids=input_ids, decoder_input_ids=decoder_input_ids, use_cache=True)
            ...
            outputs = self._model(input_ids=input_ids, decoder_input_ids=decoder_input_ids, past_key_values=self._past_key_values, use_cache=True)
    ...
    def _generate_with_kvcache(self, input_ids : torch.Tensor, decoder_input_ids : torch.Tensor, 
                                    gamma : int, 
        ...
        for _ in range(gamma):
            q = self._forward_with_kvcache(input_ids, x, use_debug)
            next_tok = sample(q)
    ...
    @torch.no_grad()
    def generate(self, input : torch.Tensor,decoder_input_ids : torch.Tensor, gamma : int) -> torch.Tensor:
        output = self._generate_with_kvcache(input, decoder_input_ids, gamma)
        return output
